...
class DirectEdit:
    ...
    def edit_files(self):
        ...
        if ed_fileCount > 0:
            response = input(
                f"Found {ed_fileCount} files with existing tags. Do you still want to edit? (y/N): "
            ).lower()
            if response != "y":
                self.status_file_msg(
                    0,
                    0,
                    f"[Editing] \x1b[31mCancelled\x1b[0m - No changes have been made",
                )
                return
        ...
        for i, file in enumerate(files):
            ...
            if data:
                ...
                self.status_file_msg(
                    i + 1,
                    len(files),
                    f"[Editing] \x1b[32mFound Data\x1b[0m {title} - {artist} - {year}",
                )
                self.edit_tags(
                ...
                self.status_file_msg(
                    i + 1, len(files), f"[Editing] \x1b[31mNo Data Found\x1b[0m"
                )
                failcount += 1
            sleep(0.05)
        ...
        self.status_file_msg(
            0,
            0,
            f"[Editing] \x1b[32mFinished\x1b[0m - \x1b[35m{len(files) - failcount}\x1b[0m files edited - \x1b[31m{failcount}\x1b[0m files failed",
        )
